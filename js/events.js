...
function saveEvent() {
  ...
  const ev = { zIdx:S.zIdx, zoneName:S.zones[S.zIdx]?.name||'', type, min, memo, time:new Date().toISOString() };
  S.events.push(ev);
  saveSt();
  addLog('a','이벤트: '+type, ft(new Date(ev.time)), min+'분'+(memo?' · '+memo:''), undefined, true);
  closeModal('m-event');
