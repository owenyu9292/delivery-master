...
function confirmZone() {
  ...
  // 현재까지 완료된 모든 구역 효율 표시
  let allZoneEff = '';
  S.results.forEach((r,i) => {
    allZoneEff += `
      <div class="card" style="margin-bottom:8px;">
        <div style="font-size:12px;font-weight:700;color:var(--text2);margin-bottom:6px;">${i+1}구역 · ${r.name}</div>
        <div class="eff-row"><span class="eff-lbl">실제 배송 효율</span><span class="eff-val" style="color:var(--accent);">${r.rEff}개/시간</span></div>
        <div class="eff-row"><span class="eff-lbl">전체 업무 효율</span><span class="eff-val" style="color:var(--accent2);">${r.tEff}개/시간</span></div>
        <div class="eff-row"><span class="eff-lbl">이벤트 보정 효율</span><span class="eff-val" style="color:var(--amber);">${r.cEff}개/시간</span></div>
      </div>`;
  });
  ...
  document.getElementById('between-summary').innerHTML = `
    <div class="stat-grid">
      <div class="stat-box"><div class="stat-lbl">완료 수량</div><div class="stat-val" style="color:var(--accent);">${doneQty}</div><div class="stat-sub">개</div></div>
      <div class="stat-box"><div class="stat-lbl">이번 구역</div><div class="stat-val" style="color:var(--accent2);">${rEff}</div><div class="stat-sub">개/시간</div></div>
    </div>
    ${allZoneEff}`;
