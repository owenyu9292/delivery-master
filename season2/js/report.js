...
// ── SAFETY BACKUP ──
function createSafetyBackup(reason, dates) {
  try {
    const createdAt = new Date().toISOString();
    const targetDates = [...new Set((dates&&dates.length ? dates : [todayKey()]).filter(Boolean))];
    const keys = new Set(['all_dates', 'all_data', 'zoneCfg', 'holiday_settings', 'gmail']);

    targetDates.forEach(d=>{
      keys.add('st_'+d);
      keys.add('logs_'+d);
      keys.add('report_'+d);
      keys.add('gmail_sent_'+d);
    });
    ...
    const key = 'safety_backup_' + createdAt.replace(/[:.]/g, '-');
    const backup = {
      version: 1,
      reason,
      createdAt,
    ...
    let index = [];
    try { index = JSON.parse(localStorage.getItem('safety_backup_index')||'[]'); }
    catch(e) { index = []; }

    index.push({ key, reason, createdAt, dates: targetDates, itemCount: Object.keys(items).length });
    while (index.length > 20) {
      const old = index.shift();
      if (old && old.key) localStorage.removeItem(old.key);
    }
    localStorage.setItem('safety_backup_index', JSON.stringify(index));

    return { ok:true, key, itemCount:Object.keys(items).length };
  } catch(e) {
...
function collectSeason2StorageItems() {
  const items = {};
  for (let i=0; i<localStorage.length; i++) {
    const key = localStorage.key(i);
    if (!key || !key.startsWith(DM2_STORAGE_PREFIX)) continue;
    items[key] = localStorage.getItem(key);
  }
  return items;
}

function downloadJsonFile(obj, filename, successMsg) {
...
async function saveJsonToPickedFolder(obj, filename) {
...
async function doFolderBackup() {
  try {
    const backup = createSafetyBackup('before_folder_backup', [todayKey()]);
    if (!backup.ok) {
      toast('자동 백업 실패 · 폴더 저장 중단');
      return;
    }

    const exportObj = buildFullBackupObject('folder_backup');
    const filename = backupFilename('폴더백업');
    const saved = await saveJsonToPickedFolder(exportObj, filename);
...
function doPhoneBackup() {
  try {
    const backup = createSafetyBackup('before_phone_file_backup', [todayKey()]);
    if (!backup.ok) {
      toast('자동 백업 실패 · 파일 저장 중단');
      return;
    }
...
function doExport() {
  try {
    const exportObj = buildFullBackupObject('manual_export');
    if (exportObj.totalDays===0 && exportObj.currentState.phase==='before') {
      toast('저장된 데이터가 없습니다');
      return;
    }
...
function doResetConfirmed() {
  const tKey = todayKey();
  const backup = createSafetyBackup('before_today_reset', [tKey]);
  if (!backup.ok) {
    toast('자동 백업 실패 · 초기화 중단');
    return;
  }
